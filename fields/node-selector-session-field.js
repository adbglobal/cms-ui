define(function(require, exports, module) {
    ...
    return UI.registerField("node-selector-session", Alpaca.Fields.SelectField.extend({
        ...
        loadCacheList: function(callback) {
            var self = this;
            var clist = null
            var cachekey = "node-selector-session:" + self.connector.branch.getUri() + self.schema._relator.nodeType
            ...
            if (clist) {
                ...
                if (sessionStorage.hasOwnProperty('region')) {
                    translationOptions.locale = sessionStorage.getItem('region');
                } else {
                    translationOptions.$exists = false;
                }
                self.connector.cache(cachekey, clist);
                self.connector.branch.queryNodes({
                    _type: self.schema._relator.nodeType,
                    "_features.f:translation": translationOptions
                }, {
                    "sort": {
                        "title": 1
                    },
